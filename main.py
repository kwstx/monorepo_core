...
def run_python_demo(module_name, script_path):
    ...
    module_dir = os.path.abspath(module_name)
    subprocess.run([sys.executable, script_path], cwd=module_dir)
...
def main():
    ...
    if not os.path.exists("venv") and not os.getenv("VIRTUAL_ENV"):
        logger.warn("Virtual environment not detected. Ensure dependencies are installed.")

    # Option 1: Run Python Logic Demos
    # These are run in their own process to avoid shadowing 'src' directories between modules
