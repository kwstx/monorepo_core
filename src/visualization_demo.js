...
function createBudget(agentId, totalBudget, spentBudget) {
    return {
        id: `budget-${agentId}`,
        agentId,
        allocations: [
...
function createHistoryPoint(budget, timestamp, spentMonetary, spentCompute) {
    return {
...
runVisualizationDemo().catch((error) => {
    console.error(error);
    process.exitCode = 1;
});
//# sourceMappingURL=visualization_demo.js.map