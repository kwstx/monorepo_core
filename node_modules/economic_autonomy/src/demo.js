...
for (const action of proposedActions) {
    ...
    console.log(` Predictive: ROI=${result.predictiveAssessment.expectedRoi.toFixed(2)}, ` +
    ...
    if (result.decision === 'allow' || result.decision === 'flag') {
        ...
        const expectedDownstreamBenefit = (action.predictedDownstreamImpact ?? []).reduce((sum, item) => sum + (item.expectedBenefit * item.probability), 0);
...
for (const entry of pnlTracker.getLedger()) {
    console.log(` - ${entry.entryId}: agent=${entry.agentId}, action=${entry.actionId}, revenue=${entry.revenue}, totalCosts=${entry.totalCosts}, netPnL=${entry.netPnL}`);
}
...
const underperfBudget = JSON.parse(JSON.stringify(exampleAgentBudget));
...
const cooperativeBudget = JSON.parse(JSON.stringify(underperfBudget));
cooperativeBudget.id = 'budget-003';
cooperativeBudget.agentId = 'agent-coop-underperf';
const coopLowPerfInput = {
    ...lowPerfInput,
