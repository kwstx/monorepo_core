...
async function main() {
    ...
    const fpSignal = optimizationEngine.createFalsePositiveSignal(decisionObject.id, scoreResult, classification, ['regulatoryExposure', 'operationalRisk'], 0.85);
    ...
    const mvSignal = optimizationEngine.createMissedViolationSignal('decision-missed-001', score2, classification2, ['regulatoryExposure', 'financialCost'], 0.75, 0.9);
    ...
    const outcomeSignal = optimizationEngine.createOutcomeSignal(decision2.id, score2, classification2, {
    ...
    const overrideSignal = optimizationEngine.createOverrideSignal(decisionObject.id, scoreResult, classification, rejectionRecord);
    ...
    const report1 = optimizationEngine.optimize(scoringEngine, classificationEngine, adaptationSignal);
    if (report1) {
        console.log(`\nOptimization Report:`);
        ...
        for (const [dim, delta] of Object.entries(report1.appliedDimensionDeltas)) {
            const direction = delta > 0 ? 'TIGHTENED' : 'RELAXED';
            console.log(`    ${dim}: ${delta.toFixed(6)} (${direction})`);
        }
    }
    ...
    for (let i = 0; i < 3; i++) {
        const goodOutcome = optimizationEngine.createOutcomeSignal(`decision-good-${i}`, scoreResult, classification, {
    ...
    const rollbackResult = optimizationEngine.rollback(v1.versionId, 'Automated safety check detected anomalous threshold drift', scoringEngine, classificationEngine);
    console.log(`\nRollback Result:`);
