...
export class TreasuryEngine {
    ...
    pnlTracker;
    options;
    constructor(pnlTracker, options = {}) {
     ...
     */
    createPool(id, name, resourceType, unit) {
        const pool = {
            id,
            name,
            totalAmount: 0,
     ...
     */
    contributeFromUnused(budget, poolId, contributionPercentage) {
        const pool = this.pools.get(poolId);
        if (!pool)
            throw new Error(`Pool ${poolId} not found.`);
        const allocation = budget.allocations.find(a => a.resourceType === pool.resourceType);
        if (!allocation)
            throw new Error(`Agent ${budget.agentId} has no allocation for ${pool.resourceType}`);
        const unused = allocation.totalBudget - (allocation.spentBudget + allocation.pendingAllocations);
        if (unused <= this.options.minContributionThreshold)
            return 0;
     ...
     */
    reconcileProject(projectId, realizedGain) {
...
//# sourceMappingURL=TreasuryEngine.js.map